...
export const getJob = (id: string) =>
  apiFetch<{ job: Job }>(`/jobs/${id}`);

export const createJob = (data: { title: string; companyId?: string; salary?: string; url?: string }) =>
  apiFetch<{ job: Job }>('/jobs', { method: 'POST', body: JSON.stringify(data) });

export const updateJob = (id: string, data: Partial<{ title: string; companyId: string; salary: string; url: string }>) =>
...
// Stages
export const getStages = (jobId: string) =>
  apiFetch<{ stages: Stage[] }>(`/stages?jobId=${jobId}`);

export const createStage = (jobId: string, data: { name: string }) =>
  apiFetch<{ stage: Stage }>('/stages', { method: 'POST', body: JSON.stringify({ ...data, jobId, order: 0 }) });
...
export const getNotes = (jobId: string) =>
  apiFetch<{ notes: Note[] }>(`/notes?jobId=${jobId}`);
...
export const updateReminder = (id: string, data: Partial<{ message: string; remindAt: string }>) =>
  apiFetch<{ reminder: Reminder }>(`/reminders/${id}`, { method: 'PUT', body: JSON.stringify(data) });
