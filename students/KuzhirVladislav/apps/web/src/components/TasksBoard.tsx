...
export default function TasksBoard() {
  ...
  // Вспомогательный компонент для рендеринга содержимого карточки
      ...
      <div className="task-content">
        ...
        {task.description && (
          <div className="task-desc" style={{
            ...
            color: 'var(--muted)',
            marginBottom: '8px',
            whiteSpace: 'pre-wrap' // Позволяет видеть переносы строк
          }}>
            {task.description}
          </div>
        )}
        ...
        <div className="task-meta"
          ...
          {task.dueDate && (
            <div className="task-due">
              📅 <strong>Дедлайн:</strong> {new Date(task.dueDate).toLocaleString('ru-RU', {
              year: 'numeric',
              month: 'long',
              day: 'numeric',
              hour: '2-digit',
  ...
  if (error) return <div className="card" style={{color: 'var(--accent-3)'}}>Ошибка: {error}</div>
          ...
          <div className="board">
            ...
            >
              <div className="board-header">
                <div style={{display: 'flex', alignItems: 'center', gap: '8px', flex: 1}}>
                  <button className="board-toggle" onClick={() => toggleSection('not-done')}>
                    <IconChevronDown
                      style={{transform: collapsedSections.has('not-done') ? 'rotate(-90deg)' : 'rotate(0deg)'}}/>
                  </button>
                  <h3>⭕ Не выполнено</h3>
                </div>
              ...
              {!collapsedSections.has('not-done') && (
                <div className="board-cards">
                  {getNotDoneTasks().map(task => <RenderTaskCard key={task.id} task={task}
                                                                 isDone={false}/>)}
            ...
            >
              <div className="board-header">
                <div style={{display: 'flex', alignItems: 'center', gap: '8px', flex: 1}}>
                  <button className="board-toggle" onClick={() => toggleSection('not-done')}>
                    <span style={{
                      display: 'inline-flex',
                      transform: collapsedSections.has('not-done') ? 'rotate(-90deg)' : 'rotate(0deg)',
              ...
              {!collapsedSections.has('done') && (
                <div className="board-cards">
                  {getDoneTasks().map(task => <RenderTaskCard key={task.id} task={task}
                                                              isDone={true}/>)}
