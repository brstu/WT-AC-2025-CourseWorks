  ...
  return (
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <h1>Клиенты</h1>
        <Link to="/clients/new"><button className="button">Создать</button></Link>
      </div>
        ...
        <div className="list">
          ...
          {items.map(c=> (
            <div key={c.id} className="item">
              ...
              <div>
                <button className="button" onClick={() => handleClientClick(c)}>Открыть</button>
              </div>
            </div>
          ))}
        </div>
